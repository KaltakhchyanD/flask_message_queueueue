...
def run_task():
    ...
    if not task_json["quantity"]:
        return {"error_code": 400, "error_message": "quantity should not be empty"}, 400
    elif not task_json["quantity"].isdigit():
        return {"error_code": 400, "error_message": "quantity should be digit"}, 400
    elif not int(task_json["quantity"])>0:
        return {"error_code": 400, "error_message": "quantity should more then 0"}, 400
    

    if not task_json["priority"]:
        return {"error_code": 400, "error_message": "priority should not be empty"}, 400
    elif not task_json["priority"].isdigit():
        return {"error_code": 400, "error_message": "priority should be digit"}, 400
    elif not 0<int(task_json["priority"])<4:
        return {"error_code": 400, "error_message": "priority should between 1 and 3"}, 400
